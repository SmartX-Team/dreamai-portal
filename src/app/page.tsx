...
export default function HomePage() {
  const items = dreamData.catalog.categories;
  ...
  function drawItem(item: Item, idx: number) {
    ...
    let element;
    if (item.imageUrl) {
      element= (<div key={idx} className={`relative ${width} ${height} ${margin}`}>
        <Image
          src={item.imageUrl}
    ...
    {/* 이미지 느낌나게 카드를 그리기 */}
      ...
      element = (
      <div key={idx} className={`bg-blue-100 p-4 rounded shadow text-center w-${width} h-${height}`}>
    ...
    <>
      ...
      <section className="bg-white py-10">
        <div className="grid grid-cols-1 md:grid-cols-3">
          ...
          {/* 우측 아이템 */}
          <div className="flex flex-col items-center">
            {rightItems.map(drawItem)}
          </div>
        </div>
      </section>

      <footer className="relative bg-zinc-900 text-zinc-300 text-xl py-8 w-full">
